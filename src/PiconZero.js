// NodeJS library for 4tronix Picon Zero (Ported from Python)
/* Picon Zero CONTROL
...
function pz() {
    ...
    this.setPixel = function () {
        bus.writeBytes(Commands.OUTPUT5, [0, 255, 0, 0],  function(err) {
            if (err) {
                throw(err);
            }
        });
    ...
    this.setBrightness = function (brightness) {
        bus.writeBytes(Commands.SETBRIGHT, brightness,  function(err) {
            if (err) {
                throw(err);
            }
        });
